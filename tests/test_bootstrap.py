...
def _run_bootstrap(args, env_extra, timeout=60):
    ...
    env.setdefault("CORTEX_SKIP_SYNC", "1")
...
def test_bootstrap_help_exits_zero():
    ...
    assert "cortex-bootstrap" in result.stdout
    assert "Exit codes" in result.stdout


def test_bootstrap_writes_capabilities(tmp_path):
    """A full shim run with --doctor should populate ~/.cortex/.capabilities."""
    ...
    caps_file = cortex_dir / ".capabilities"
    ...
    assert "writable_cortex_dir" in data
    assert data["writable_cortex_dir"] is True  # tmp_path is always writable


def test_bootstrap_refuses_unwritable_cortex_dir(tmp_path):
    """Bootstrap must hard-fail (exit 13) when the cortex dir is not writable."""
    # Create a read-only parent so mkdir will succeed but canary will fail
    readonly_parent = tmp_path / "readonly"
    readonly_parent.mkdir()
    cortex_dir = readonly_parent / ".cortex"
    cortex_dir.mkdir()
    os.chmod(cortex_dir, 0o555)  # r-x for owner
    ...
    try:
        ...
        os.chmod(cortex_dir, 0o755)  # restore for cleanup


def test_bootstrap_skip_sync_env_var(tmp_path):
    """CORTEX_SKIP_SYNC=1 must bypass the uv sync step entirely."""
    cortex_dir = tmp_path / ".cortex"
    vault = tmp_path / "obsidian-brain"
    (vault / "cortex" / "memories").mkdir(parents=True)
    ...
    log_file = cortex_dir / "bootstrap.log"
    assert log_file.exists()
    log_content = log_file.read_text()
    assert "CORTEX_SKIP_SYNC" in log_content or "skipping uv sync" in log_content


def test_capabilities_module_roundtrip(tmp_path, monkeypatch):
    """In-process roundtrip: write + read + pretty_report."""
    monkeypatch.setenv("CORTEX_DIR", str(tmp_path / ".cortex"))
    monkeypatch.setenv("CORTEX_OBSIDIAN_VAULT", str(tmp_path / "vault"))
    (tmp_path / "vault" / "cortex" / "memories").mkdir(parents=True)

    import importlib

    from cortex_server import capabilities, models
    importlib.reload(models)
    importlib.reload(capabilities)

    caps = capabilities.detect_capabilities()
    assert caps["schema_version"] == 1
    assert caps["writable_cortex_dir"] is True
    ...
    assert read_back["writable_cortex_dir"] is True

    report = capabilities.pretty_report(caps)
    assert "Cortex Doctor" in report
