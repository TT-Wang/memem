...
# ---------------------------------------------------------------------------
# Test 3: SessionStart slice surfaces pending suggestions (oldest-first)
# ---------------------------------------------------------------------------


def test_session_start_slice_surfaces_pending(tmp_vault, tmp_cortex_dir, monkeypatch):
    """3 pending procedural suggestions appear in the render block, oldest first."""
    import memem.obsidian_store as obs
    importlib.reload(obs)
    ...
    old_mem = _make_procedural_mem(
        title="Suggestion 1",
        reason="Stop adding boilerplate",
        created_iso=(now - timedelta(days=5)).isoformat(),
    )
    mid_mem = _make_procedural_mem(
        title="Suggestion 2",
        reason="Don't repeat context",
        created_iso=(now - timedelta(days=3)).isoformat(),
    )
    new_mem = _make_procedural_mem(
        title="Suggestion 3",
        reason="Use concise answers",
    ...
    try:
        ...
        result = _render_procedural_suggestions_block()
    finally:
        obs_mod._obsidian_memories = original_fn

    assert "Pending instruction rewrites" in result, f"Section missing from output:\n{result!r}"
    # All 3 reasons should appear (oldest-first sorting)
    assert "Stop adding boilerplate" in result
    assert "Don't repeat context" in result
    assert "Use concise answers" in result

    # Verify oldest-first ordering: "Stop adding boilerplate" (5 days old) must appear before
    # "Use concise answers" (1 day old) in the output text
    pos_old = result.index("Stop adding boilerplate")
    pos_new = result.index("Use concise answers")
    assert pos_old < pos_new, "Suggestions should be oldest-first"


# ---------------------------------------------------------------------------
# Test 4: SessionStart omits section when no pending suggestions
# ---------------------------------------------------------------------------


def test_session_start_slice_omits_section_when_no_pending(tmp_vault, tmp_cortex_dir, monkeypatch):
    """No pending procedural suggestions → section is absent from output."""
    import memem.obsidian_store as obs
    importlib.reload(obs)

    # Patch to return no procedural-suggestion memories
    import memem.active_slice_engine as ase

    monkeypatch.setattr(ase, "_render_procedural_suggestions_block", lambda: "")
    ...
    result = generate_session_start_slice(
        scope_id="general",
        session_id="test-sess-no-pending",
        memem_dir=str(tmp_cortex_dir),
    )

    assert "Pending instruction rewrites" not in result, (
        f"Section should be absent when no pending suggestions:\n{result!r}"
    )
